  ...
  return (
    <div className={`flex flex-col gap-2 relative ${className}`} ref={ref}>
      ...
      <button
        type="button"
        onClick={() => setOpen((o) => !o)}
        className={`h-12 flex items-center justify-between px-[17px] border text-left ${
        ...
        <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-grey-200 shadow-lg z-30 max-h-60 overflow-y-auto">
          {options.map((opt) => (
            <button
              ...
              className="w-full text-left px-[17px] py-3 text-[15px] text-brand-blue hover:bg-grey-50"
            >
              {opt}
            </button>
          ))}
