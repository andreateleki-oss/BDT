...
function SurfaceInput({ label, value, onChange, unit, onUnitChange, className = "" }) {
  ...
  return (
    <div className={`flex flex-col gap-2 ${className}`}>
      ...
      <div className="h-12 bg-grey-50 border border-grey-200 flex items-center px-[17px] gap-2 relative" ref={ref}>
        <input
          type="number"
          min="0"
          value={value}
        ...
        <button
          type="button"
          onClick={() => setOpen((o) => !o)}
          className="border-l border-grey-200 pl-2 flex items-center gap-1 text-grey text-[15px] shrink-0"
        >
          {unit}
          ...
          <div className="absolute top-full right-0 mt-1 bg-white border border-grey-200 shadow-lg z-30 w-20">
            {UNITS.map((u) => (
              <button
                ...
                className="w-full text-left px-3 py-2 text-[15px] text-brand-blue hover:bg-grey-50"
              >
                {u}
              </button>
            ))}
